...
test:
	@coverage run test_project/manage.py test incuna_forms --verbosity=${VERBOSITY}
	@flake8 .
...
migrate:
	@if [ `psql -t -c "SELECT COUNT(1) FROM pg_catalog.pg_database WHERE datname = 'incuna_forms'"` -eq 0 ]; then \
		psql -c "CREATE DATABASE incuna_forms"; \
	fi
